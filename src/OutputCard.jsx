  ...
  return (
    <div className={`flex flex-col justify-start items-center w-full min-h-48 max-h-48 h-48 md:min-h-96 md:max-h-96 md:h-full bg-gray-300 border border-gray-200 rounded-lg basis-1/3 ml-4 mr-4 p-2`}>
        ...
        <div className='rounded-xl flex flex-col justify-center items-center w-full h-full'
              style={outputStyle}>
          <a href={btnActive ? btnLink : '' } onClick={handleButtonClick} className={`fh-button${btnStyle !== '' ? `-${btnStyle}` : ``}-pop fh-fixed--${btnLocation} ${(isLeft && (btnLocation !== 'side')) ? `overrideLeft-class` : ''} ${(isLeft && (btnLocation === 'side')) ? `overrideLeftSIDE-class` : ''} ${applyGradient ? 'overrideGradient-class' : ''} fh-font--inherit fh-shape--${btnShape} fh-size--${btnSize}  fh-icon--${btnIcon} ${btnVisibility === 'bothDesktopAndMobile' ? '':`fh-hide--${btnVisibility}`}`} 
